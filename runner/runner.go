...
func (r *Runner) Run(ctx context.Context) error {
	...
	cmdPath := filepath.Join(r.directory, "run.sh")
	args := []string{"--jitconfig", r.config}
	r.logger.Debug().Msgf("Executing runner command: %s %v", cmdPath, args)

	runCmd := exec.CommandContext(ctx, cmdPath, args...)
	...
	r.logger.Debug().Msgf("Runner environment variables: %v", runCmd.Env)
